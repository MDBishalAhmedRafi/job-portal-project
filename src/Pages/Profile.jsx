...
<div className="min-h-screen bg-gradient-to-br from-blue-100 via-white to-purple-100 flex items-center justify-center p-6 lg:w-11/12 lg:mx-auto mx-2 rounded-2xl">
      <div className="bg-white shadow-2xl rounded-3xl p-8 max-w-sm w-full text-center border border-gray-200">
        <div className="relative inline-block">
          <img
            className="w-28 h-28 rounded-full mx-auto border-4 border-green-400 shadow-lg"
            src={user.photoURL}
            alt="User avatar"
          />
        ...
        <h2 className="mt-4 text-2xl font-bold text-gray-800">Name: {user.displayName}</h2>
        <p className="text-gray-500 mb-6">User Email: <span className='font-bold'>{user.email}</span></p>
                  <Link to="/update-profile">
        <button className="cursor-pointer mt-4 bg-green-600 text-white px-5 py-2 rounded-xl shadow-md hover:bg-green-700 transition duration-200">
          Update Profile
        </button>
        </Link>
      </div>
