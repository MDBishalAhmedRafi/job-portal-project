  ...
  return (
    <div className="mx-2 p-6">
      <div className="bg-white shadow-lg rounded-2xl p-8 mb-10 flex flex-col md:flex-row items-center gap-6">
        <img src={singleCompany.logo} alt="Company Logo" className="w-32 h-32 object-cover rounded-full border-4 border-primary" />
        <div>
          <h1 className="text-4xl font-extrabold mb-2 text-primary">{singleCompany.name}</h1>
          ...
          <a href={singleCompany.website} target="_blank" rel="noopener noreferrer" className="btn btn-outline btn-primary">
      ...
      <div className="space-y-6">
        {singleCompany.jobs.map((job) => (
          <div key={job.id} className="bg-gradient-to-r from-blue-50 to-purple-100 p-6 rounded-xl shadow-md border border-blue-200">
            <h3 className="text-2xl font-bold text-purple-800">{job.title}</h3>
            <p className="text-sm text-gray-600">Type: {job.jobType}</p>
            <p className="text-sm text-gray-700">💰 Salary: {job.salary}</p>
            <button className="btn btn-primary mt-4" onClick={() => openModal(job)}>
              View Details
            </button>
          </div>
        ))}
      ...
      <dialog id="job_modal" className="modal">
        <div className="modal-box">
          {selectedJob && (
            <>
              <h3 className="font-bold text-2xl text-primary mb-3">{selectedJob.title}</h3>
